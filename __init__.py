...
# 分别注册不同的排行榜命令
gold_rank = on_command(
    "金币排行", permission=GROUP, priority=5, block=True
)

win_rank = on_command(
    "胜场排行", aliases={"胜利排行"}, permission=GROUP, priority=5, block=True
)

lose_rank = on_command(
    "败场排行", aliases={"失败排行"}, permission=GROUP, priority=5, block=True
)

european_rank = on_command(
    "欧洲人排行", permission=GROUP, priority=5, block=True
)

philanthropist_rank = on_command(
    "慈善家排行", permission=GROUP, priority=5, block=True
)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    # 检查该群组是否有数据
    if group_id not in manager._player_data or not manager._player_data[group_id]:
        await gold_rank.finish("该群暂无游戏数据")

    # 获取该群所有玩家数据
    players = manager._player_data[group_id]

    # 按gold字段降序排序
    sorted_players = sorted(
        players.values(),
        key=lambda x: x.get("gold", 0),
        reverse=True  # 降序排列，金币多的在前
    )

    # 生成排行榜文本
    rank_text = "\t金币排行榜\n"
    for i, player in enumerate(sorted_players[:10], 1):  # 只显示前10名
        rank_text += f"{i}. {player['nickname']}：{player['gold']}\n"

    await gold_rank.finish(rank_text.rstrip())
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    # 检查该群组是否有数据
    if group_id not in manager._player_data or not manager._player_data[group_id]:
        await win_rank.finish("该群暂无游戏数据")
    ...
    sorted_players = sorted(
        players.values(),
        key=lambda x: x.get("win_count", 0),
        reverse=True
    )
    ...
    for i, player in enumerate(sorted_players[:10], 1):
        win_count = player.get("win_count", 0)
        lose_count = player.get("lose_count", 0)
        total_games = win_count + lose_count
        win_rate = (win_count / total_games * 100) if total_games > 0 else 0

        rank_text += f"{i}. {player['nickname']}：{win_count}胜"
        if total_games > 0:
            rank_text += f"（胜率{win_rate:.1f}%）"
        rank_text += "\n"

    await win_rank.finish(rank_text.rstrip())
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    # 检查该群组是否有数据
    if group_id not in manager._player_data or not manager._player_data[group_id]:
        await lose_rank.finish("该群暂无游戏数据")
    ...
    sorted_players = sorted(
        players.values(),
        key=lambda x: x.get("lose_count", 0),
        reverse=False
    )
    ...
    for i, player in enumerate(sorted_players[:10], 1):
        ...
        rank_text += f"{i}. {player['nickname']}：{lose_count}败"
        if total_games > 0:
            rank_text += f"（败率{lose_rate:.1f}%）"
        rank_text += "\n"

    await lose_rank.finish(rank_text.rstrip())


@european_rank.handle()
async def _(bot: Bot, event: GroupMessageEvent):
    """欧洲人排行榜实现"""
    group_id = str(event.group_id)
    manager = russian_manager
    ...
    # 检查该群组是否有数据
    if group_id not in manager._player_data or not manager._player_data[group_id]:
        await european_rank.finish("该群暂无游戏数据")

    # 获取该群所有玩家数据
    players = manager._player_data[group_id]

    # 按make_gold字段降序排序（累计获得金币越多越欧）
    sorted_players = sorted(
        players.values(),
        key=lambda x: x.get("make_gold", 0),
        reverse=True  # 降序排列，获得金币多的在前
    )

    # 生成排行榜文本
    rank_text = "\t欧洲人排行榜\n"
    for i, player in enumerate(sorted_players[:10], 1):  # 只显示前10名
        rank_text += f"{i}. {player['nickname']}：已赢得{player['make_gold']}金币\n"

    await european_rank.finish(rank_text.rstrip())


@philanthropist_rank.handle()
async def _(bot: Bot, event: GroupMessageEvent):
    """慈善家排行榜实现"""
    group_id = str(event.group_id)
    manager = russian_manager
    ...
    # 检查该群组是否有数据
    if group_id not in manager._player_data or not manager._player_data[group_id]:
        await philanthropist_rank.finish("该群暂无游戏数据")

    # 获取该群所有玩家数据
    players = manager._player_data[group_id]

    # 按lose_gold字段升序排序（输掉金币越少越好）
    sorted_players = sorted(
        players.values(),
        key=lambda x: x.get("lose_gold", 0),
        reverse=True  # 升序排列，输掉金币少的在前
    )

    # 生成排行榜文本
    rank_text = "\t慈善家排行榜\n"
    for i, player in enumerate(sorted_players[:10], 1):  # 只显示前10名
        rank_text += f"{i}. {player['nickname']}：已输掉{player['lose_gold']}金币\n"

    await philanthropist_rank.finish(rank_text.rstrip())
...
async def _(
    ...
    if at_:
        ...
        msg = (
            f"{player1_name} 向 {MessageSegment.at(at_)} 发起了决斗！请 {at_player_name} 在3分钟内回"
            f"复‘接受对决’ or ‘拒绝对决’，超时此次决斗作废！"
        )
    else:
        at_ = 0
        msg = "若3分钟内无人接受挑战则此次对决作废【首次游玩请发送 ’俄罗斯轮盘帮助‘ 来查看命令】"
