...
#
#     # 检查群组数据是否存在
#     if group_id not in player_data or not player_data[group_id]:
#         return "该群暂无游戏数据"
#
...
#     rank_config = {
#         "gold_rank": ("\t金币排行榜\n", "gold", True),  # 金币越多越好
#         "win_rank": ("\t胜场排行榜\n", "win_count", True),  # 胜场越多越好
#         "lose_rank": ("\t败场排行榜\n", "lose_count", False),  # 败场越少越好
#         "make_gold": ("\t赢取金币排行榜\n", "make_gold", True),  # 赢取金币越多越好
#         "lose_gold": ("\t输掉金币排行榜\n", "lose_gold", False)  # 输掉金币越少越好
#     }
#
#     if type_ not in rank_config:
#         return "未知的排行榜类型"
#
#     rank_name, data_key, reverse = rank_config[type_]
#
#     # 创建用户数据列表，包含用户ID和数据值
#     user_data_list = []
#     for user_id in all_user:
#         user = player_data[group_id][user_id]
#         # 确保数据键存在，不存在则设为0
#         value = user.get(data_key, 0)
#         user_data_list.append((user_id, value, user["nickname"]))
#
#     # 根据reverse参数排序：True为降序(数值大的在前)，False为升序(数值小的在前)
#     user_data_list.sort(key=lambda x: x[1], reverse=reverse)
#
#     # 生成排行榜文本
#     rst = ""
#     for i, (user_id, value, nickname) in enumerate(user_data_list[:10], 1):
#         rst += f"{i}. {nickname}：{value}\n"
#
#     # 如果rst为空，说明没有数据
#     if not rst:
#         return rank_name + "暂无数据"
#
#     return rank_name + rst.rstrip()
...
class RussianManager:
    ...
    def accept(self, event: GroupMessageEvent) -> Union[str, Message]:
        ...
        if time.time() - self._current_player[event.group_id]["time"] > 180:
    ...
    async def check_current_game(
        ...
        if (
            ...
            and time.time() - self._current_player[event.group_id]["time"] <= 180
        ):
